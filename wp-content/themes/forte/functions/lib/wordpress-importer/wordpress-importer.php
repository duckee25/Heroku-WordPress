...
Modified for Forte theme by Manuel Masia | Pixedelic
Version 0.6.1
*/
...
<?php
	...
	function process_menu_item( $item ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			if ( isset($_pix_megamenu_item) ) update_post_meta( (int) $id, '_pix_megamenu_item', $_pix_megamenu_item );
			if ( isset($_pix_column_item) ) update_post_meta( (int) $id, '_pix_column_item', $_pix_column_item );
			if ( isset($_pix_icon_item) ) update_post_meta( (int) $id, '_pix_icon_item', $_pix_icon_item );
			if ( isset($_pix_image_item) ) update_post_meta( (int) $id, '_pix_image_item', $_pix_image_item );
			if ( isset($_pix_width_item) ) update_post_meta( (int) $id, '_pix_width_item', $_pix_width_item );
			if ( isset($_pix_title_item) ) update_post_meta( (int) $id, '_pix_title_item', $_pix_title_item );
...
function pix_wordpress_importer_init() {
...
add_action( 'admin_init', 'pix_wordpress_importer_init' );
