...
function select_fonts(){
	global $options, $current_user, $display_name;

	if ($_GET['page']=='select_fonts') { 
?>
...
<div id="forte_dynamic_tab">
        	...
        	<div id="forte_content_title">
            	Typography: <small>Google fonts list</small>
            ...
            <form method="post" class="dynamic_form" action="<?php echo admin_url("admin.php?page=admin_interface"); ?>" id="select_typo">
                      
                <label for="pix_enable_google">Enable Google fonts</label>
                <input type="hidden" name="pix_enable_google" value="0">
                <input type="checkbox" name="pix_enable_google" value="true" <?php if(pix_esc_option('pix_enable_google')=='true') { echo 'checked="checked"'; } ?>>  

                <div class="tip_info_wrap">
                  <small class="tip_info">As you can see here below the list of fonts available on the Google repository is huge. To save time I enabled a few of them only, but you can decide to enable more of them or change the fonts I chose.<br><br>
                    When you want to change the font of a particular element (for instance the site title) you must click the button called "Load the Google fonts". After that the fonts you chose here below will be loaded to be selected. <strong>As more fonts you choose here below, as longer you must wait to load them</strong><br><br>
                    For the complete visual list go to the official site: <a href="http://www.google.com/webfonts" target="_blank">http://www.google.com/webfonts</a></small>
                </div><!-- .tip_info_wrap -->
<br>
            <input type="hidden" name="pix_select_fonts" value="">
<?php
	foreach ( $decoded->items as $item )
	{ ?>
    <span class="checkboxes_font">
		<span class="alignleft"><input type="checkbox" name="pix_select_fonts[]" value="<?php echo $item->family; ?>" <?php if (is_array(pix_get_option('pix_select_fonts')) && in_array($item->family,pix_get_option('pix_select_fonts'))) { echo ' checked="checked"'; } ?>></span>
		<span class="alignleft font_preview select_font_family" data-font="<?php echo $item->family; ?>" data-webfont="<?php echo str_replace(' ','+',$item->family); ?>"> <?php echo $item->family; ?></span>
