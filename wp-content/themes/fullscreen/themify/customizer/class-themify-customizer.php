...
class Themify_Customizer {
	...
	function __construct() {
		...
		add_action('wp_ajax_themify_plupload', array( $this, 'customizer_import'));
	}
	...
	function customizer_import() {
		...
		if( 'zip' == $ext[1] || 'rar' == $ext[1] || 'plain' == $ext[1] ){
			...
			if( 'zip' == $ext[1] || 'rar' == $ext[1] ) {
				...
				if( $wp_filesystem->exists( THEME_DIR . '/customizer_export.txt' ) ){
					$data = $wp_filesystem->get_contents( THEME_DIR . '/customizer_export.txt' );
					$this->set_data( unserialize( $data ) );
					$wp_filesystem->delete(THEME_DIR . '/customizer_export.txt');
					$wp_filesystem->delete($file['file']);
				} else {
					_e('Data could not be loaded1', 'themify');
				}
			} else {
				if( $wp_filesystem->exists( $file['file'] ) ){
					...
					_e('Data could not be loaded2', 'themify');
				}
			}
