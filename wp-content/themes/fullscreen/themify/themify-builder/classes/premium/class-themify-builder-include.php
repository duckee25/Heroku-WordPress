...
class Themify_Builder_Include {
	...
	public function __construct( Themify_Builder $builder ) {
            ...
            add_filter('themify_builder_subrow_fields',array($this,'subrow_styling_fields'),10,1);
		...
		$props = array(
			...
			array(
					...
					'meta' => array(
							...
							array('value' => 'builder-zoom-scrolling', 'name' => __('Zoom Scrolling', 'themify')),
							array('value' => 'builder-bg-fixed', 'name' => __('Fixed', 'themify'))
					),
	...
	public function set_layout_ajaxify() {
		...
		if ( ! empty( $builder_data ) ) {
			$GLOBALS['ThemifyBuilder_Data_Manager']->save_data( $builder_data, $current_builder_id, true );
