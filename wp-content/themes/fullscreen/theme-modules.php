...
/**
 * Show Content on Static Home Page
 *
 * @param array $data
 *
 * @return string
 */
function themify_theme_static_home_content( $data = array() ) {
	/**
	...
	$html .= sprintf( '<p class="hide-if-none"><label for="%1$s"><input type="checkbox" id="%1$s" name="%1$s" %2$s /> %3$s</label></p>',
		'setting-static_home_content',
		checked( themify_get( 'setting-static_home_content' ), 'on', false ),
		__( 'Show content when a static page is set as Front page.', 'themify' )
	);
	...
	function themify_footer_slider($data=array()){
		...
		if ( isset( $data['setting-footer_slider_auto'] ) && !$data['setting-footer_slider_auto'] ) $data['setting-footer_slider_auto'] = 'yes';
		if ( isset( $data['setting-footer_slider_autotimeout'] ) && !$data['setting-footer_slider_autotimeout'] ) $data['setting-footer_slider_autotimeout'] = 5;
		$display_options = array(
		...
		$output = '<p>
						...
						foreach($galleries as $gallery){
							if ( $gallery['value'] == themify_get( 'setting-default_bg_gallery' ) ) {
		...
		$output .= '<p>
						...
						foreach($display_options as $option => $value){
							if ( $value == themify_get( 'setting-show_caption' ) ) {
								$output .= '<option value="'.$value.'" selected="selected">'.$option.'</option>';
							} else {
								$output .= '<option value="'.$value.'">'.$option.'</option>';
							}
		...
		$output .= '<p>
						...
						foreach($autotimeout_options as $option){
							if ( $option == themify_get( 'setting-footer_slider_autotimeout' ) ) {
								$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
							} else {
								$output .= '<option value="'.$option.'">'.$option.'</option>';
							}
		...
		$output .= '<p>
						...
						foreach($speed_options as $name => $val){
							if ( themify_get( 'setting-footer_slider_speed' ) == $val ) {
		...
		$output .= '<p>';
			...
			$output .= '<span class="pushlabel">';
				...
				// Best Fit
				$output .= '<input ' . checked( isset($data[$key])? $data[$key] : 'best-fit', 'best-fit', false) . ' type="radio" id="'.$key.'_best-fit" name="'.$key.'" value="best-fit" /> ';
...
<span class="pushlabel vertical-grouped"><label for="setting-excerpt_more"><input type="checkbox" value="1" id="setting-excerpt_more" name="setting-excerpt_more" '.checked( themify_get( 'setting-excerpt_more' ), 1, false ).'/> ' . __('Display more link button in excerpt mode as well.', 'themify') . '</label></span><br />
						<span class="pushlabel"><small>' . __('Note: more text is only available if display=content and the post has the <a href="https://themify.me/docs/more-tag" target="_blank">more tag</a>', 'themify') . '</small></span>
					</p>';
