...
function themify_theme_enqueue_scripts(){
	...
	wp_enqueue_script( 'backstretch', THEMIFY_URI . '/js/backstretch.themify-version.js', array('jquery'), false, true );
	...
	wp_enqueue_script( 'gallery-script',	THEME_URI . '/js/themify.gallery.js', array('jquery','backstretch'), false, true );
	...
	function create_controller(){
		...
		if($images){
			...
			foreach( $images as $image ){
				...
				$image_data = wp_get_attachment_image_src( $image->ID, apply_filters( 'themify_theme_background_gallery_image_size', $image_size ) );
...
/**
 * Remove Builder panel for background gallery post type since it's not a user-visible type.
 *
 * @since 1.5.9
 *
 * @param array $panels All panels to add to meta box.
 *
 * @return mixed
 */
function themify_theme_builder_exclude_background_gallery( $panels ) {
	global $typenow;
	if ( empty( $typenow ) && isset( $_GET['post'] ) ) {
		$typenow = get_post_type( $_GET['post'] );
	}
	if ( 'background-gallery' == $typenow ) {
		$index = 0;
		foreach ( $panels as $panel ) {
			if ( 'background-gallery' ==  $panel['pages'] ) {
				unset( $panels[$index] );
			}
			$index++;
		}
...
add_filter( 'themify_do_metaboxes', 'themify_theme_builder_exclude_background_gallery', 12 );
