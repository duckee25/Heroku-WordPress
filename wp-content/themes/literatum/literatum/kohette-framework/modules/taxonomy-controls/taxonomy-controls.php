<?php
/**
 * Taxonomy controls module.
 *
 * Classes and functions to control and extend the Wordpress taxonomy feature
 *
 */
...
function update_taxonomy_meta($tax_id, $meta_key, $meta_value, $prev_value = '') {
	@Taxonomy::update_wpdb();
	return update_metadata('taxonomy', $tax_id, $meta_key, $meta_value, $prev_value);
}

function get_taxonomy_meta($tax_id, $key = '', $single = false) {
	@Taxonomy::update_wpdb();
	return get_metadata('taxonomy', $tax_id, $key, $single);
}

function add_taxonomy_meta($tax_id, $meta_key, $meta_value, $unique = false) {
	@Taxonomy::update_wpdb();
...
class KTT_new_taxonomy_meta {
    ...
    function KTT_new_taxonomy_meta($args) {
    ...
    function fields_html($term, $taxonomy = '' ) {
            $value = @get_taxonomy_meta( $term->term_id, $this->taxmeta_id, true );
